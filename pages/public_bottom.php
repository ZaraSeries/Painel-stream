				...
				if (mysql_num_rows($select)==0) {
					...
					while($data = mysql_fetch_array($select)) {
						echo '<tr>
							...
							<td><a href="http://'.$setting['host_add'].':'.$data['portbase'].'/" target="_blank">'.$data['owner'].'</a></td><td>
				';
						if (shoutcastcheck("".$setting['host_add']."", "".$data['portbase']."", 1)) {
							echo "<a class=\"selector\" href=\"http://".$setting['host_add'].":".$data['portbase']."/listen.pls\" TITLE=\"Sendung anh&ouml;ren\">".$messages["431"]."</a>";
						}
						else {
							echo "<a class=\"delete\" href=\"#\" TITLE=\"Auf diesem Server l&auml;uft im Moment leider keine Sendung!\">".$messages["432"]."</a>";
						}
						echo '</td></tr>';
					}
				}
